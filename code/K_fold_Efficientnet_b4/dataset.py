...
class MaskSplitByStratifiedProfileDataset(MaskBaseDataset):
    """
        train / val 나누는 기준을 이미지에 대해서 random 이 아닌
        사람(profile)을 기준으로 나눕니다.
        구현은 val_ratio 에 맞게 train / val 나누는 것을 이미지 전체가 아닌 사람(profile)에 대해서 진행하여 indexing 을 합니다
        이후 `split_dataset` 에서 index 에 맞게 Subset 으로 dataset 을 분기합니다.
    """
    def __init__(self, data_dir, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2, val_ind=0):
        self.indices = defaultdict(list)
        self.val_ind = val_ind
        super().__init__(data_dir, mean, std, val_ratio)

    @staticmethod
    def _split_profile(profiles, val_ratio, val_ind_):
        unq_csv = '/opt/ml/input/data/train/unique_group.csv'
        unq_pd = pd.read_csv(unq_csv)
        val_indices = set(unq_pd.index[unq_pd['groups'] == val_ind_]) #
    ...
    def setup(self):
        profiles = os.listdir(self.data_dir)
        profiles = sorted([profile for profile in profiles if not profile.startswith(".")])
        split_profiles = self._split_profile(profiles, self.val_ratio, self.val_ind)
